...
namespace OrmTxcSql.SqlClient.Daos
{
        ...
        where TEntity : SqlEntity, new()
    {
        ...
        protected virtual void BuildDeleteByPkCommand(SqlCommand command, TEntity entity)
        {
            // コマンドの準備に必要なオブジェクトを生成する。
            var builder = new StringBuilder();
            //
        ...
        private void BuildDeleteCommand(SqlCommand command, TEntity entity, string filter)
        {
            // コマンドの準備に必要なオブジェクトを生成する。
            var columnStringBuilder = new StringBuilder();
            //
